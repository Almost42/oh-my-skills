...
- [ ] 改动范围是否超出了 `req.md`（或 Requirement Scope）所列的需求？
...
对 multi-file spec：将 execution package 详情写入 `impl.md`，在 `index.md` 的 "Execution Package Summary" 填写摘要（文件数、影响范围、回滚目标），不在 `index.md` 重复完整内容。
