...
- `auto`：验收或会话收口发现候选时执行，不等待用户；低风险、高置信度条目直接固化。没有候选时返回“无新增经验”，不扫描全量知识。
...
只读取新增或受当前 spec 影响的 `docs/ai/knowledge/lessons/*`、验收结果和目标 owner。按经验编号或规则指纹合并重复条目，抽象掉需求名、一次性字段和临时方案，并检查现有规则冲突；不得为一次无新增经验的验收全量扫描。
