...
将修复建议写入当前 spec 的"工作流记录 / 修复提案"区域（旧文档可兼容 `Workflow Notes` / `Repair Proposal`），至少补齐：
...
**当前节点**: ...
**最近确认节点**: ...
**识别问题类型**: 设计修复（DesignRepair） | 需求修复（RequirementRepair） | 实施修复（ImplementationFix） | 验证修复（VerificationRepair）
**建议回退目标**: ...
**需更新文档**:
- ...

**代码处理建议**:
- 保留现状（preserve） | 局部回滚（partial revert） | 整体回滚（full revert）

**下一步建议**: 请先确认是否按提案回退或修复，然后再继续后续流程
**是否等待用户确认**: 是
```
